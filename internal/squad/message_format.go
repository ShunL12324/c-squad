...
Incoming C-Squad messages have a short header containing message_id, task_id when present, and recipient_generation. Claude also provides a native cross-session sender envelope; Codex includes from in the header. Acknowledge each message_id through message ack before acting, including duplicates; read the ledger instead of repeating completed work. Answer requests using reply MESSAGE --text TEXT. Reply only when a response is needed, never to a pure acknowledgment. Route all team replies through the C-Squad CLI, not native SendMessage: this preserves task tracking and reaches both Claude and Codex members. Native peer addresses are transport identities, not CLI member names. Existing JSON-formatted messages use the same acknowledgment rules.`
