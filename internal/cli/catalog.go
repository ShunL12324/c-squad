...
func definitions() []definition {
	...
	for _, op := range []string{"inspect", "claim", "approve", "merge", "reopen", "abort-merge", "brief"} {
		defs = append(defs, definition{path: "task " + op, args: " TASK", summary: map[string]string{"inspect": "Inspect one task", "claim": "Claim an available open task", "approve": "Approve the submitted candidate", "merge": "Merge an approved candidate", "reopen": "Reopen a submitted task for changes", "abort-merge": "Clear a safe-to-abort merge intent", "brief": "Ask master to summarise a task, changing nothing"}[op], min: 1, max: 1, complete: "task"})
	}
