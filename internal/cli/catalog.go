...
func addFlags(cmd *cobra.Command, names []string) {
	...
	for _, name := range names {
		...
		switch name {
		case "detach", "fresh", "full", "code", "all", "strict", "popup", "dry-run", "discard-ignored", "reprofile", "yes", "check":
		...
		if name == "name" && (cmd.Name() == "attach" || cmd.Name() == "resume" || cmd.Name() == "board" || cmd.Name() == "stop" || cmd.Name() == "recover" || cmd.Name() == "repin" || cmd.Name() == "ui") {
...
func definitions() []definition {
	defs := []definition{
		...
		{path: "update", summary: "Update csquad through the package manager that installed it; running teams keep their build until resumed", flags: []string{"check", "yes"}, example: "  csquad update --check\n  csquad update"},
		...
		{path: "task assign", args: " TASK", summary: "Assign the task owner and collaborators", min: 1, max: 1, complete: "task", flags: []string{"owner", "to", "cc"}, required: []string{"owner"}, example: "  csquad task assign T7 --owner dev --to reviewer --cc lead,docs"},
		...
		{path: "task cancel", args: " TASK", summary: "Cancel a task as master, keeping its workspace and history", min: 1, max: 1, complete: "task", flags: []string{"reason"}, required: []string{"reason"}, example: "  csquad task cancel T7 --reason 'Superseded by T9'"},
