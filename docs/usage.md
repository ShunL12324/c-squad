...
The member card's colored stripe identifies the current session even when the
keyboard cursor moves elsewhere. The wheel scrolls independently of selection.
Member names always use their assigned colors, including unselected members.
Task cards and details scroll within the task pane; task filters and the
full-width View details action share their rendered bounds with mouse targets.
